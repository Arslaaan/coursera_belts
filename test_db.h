...
void TestsMyCustom()
{
    ...
    }
    {
        Database db;
        db.Add({0, 2, 3}, "Kolya");
        db.Add({1, 12, 12}, "Kolya");
        db.Add({999, 12, 12}, "Kolya");
        db.Add({1000, 12, 12}, "Kolya");
        db.Add({9999, 12, 12}, "Kolya");
        ostringstream out;
        db.Print(out);
        AssertEqual("0000-02-03 Kolya\n0001-12-12 Kolya\n0999-12-12 Kolya\n1000-12-12 Kolya\n9999-12-12 Kolya\n", out.str(), "Order of insertion");
    }
    {
        Database db;
        db.Add({0, 2, 3}, "Kolya");
        db.Add({1, 12, 12}, "Kolya");
        db.Add({1, 12, 12}, "Sarkazi");
        db.Add({1000, 12, 12}, "Kolya");
        db.Add({9999, 12, 12}, "Kolya");
        ostringstream out;
        db.Print(out);
...

    {
        ...
        AssertEqual(Record({2019, 12, 3}, "f"), db.Last({2019, 12, 4}), " My test 6");
        ...
        AssertEqual(Record({2019, 12, 3}, "f"), db.Last({2019, 12, 4}), " My test 8");

        db.Add({2019, 12, 3}, "m");
        AssertEqual(Record({2019, 12, 3}, "m"), db.Last({2019, 12, 3}), " My test 9");
...

    {
        ...
        AssertEqual(Record({2019, 12, 1}, "c"), db.Last({2019, 12, 1}), " My test 20");

        db.Add({2019, 12, 1}, "b");
        AssertEqual(Record({2019, 12, 1}, "c"), db.Last({2019, 12, 1}), " My test 21");
...
//        cerr << db.FindIf(empty_predicate) << endl;
    ...
    // Last
    {
        ...
        db.Add(d1, "e1");
        db.Add(d2, "e2");
        AssertEqual(db.Last(d), Record({2018, 12, 22}, "e2"), "Db Last 1");
        Date d3(2018, 12, 24);
        db.Add(d3, "e3");
        AssertEqual(db.Last(d), Record({2018, 12, 24}, "e3"), "Db Last 2");
        ...
        AssertEqual(db.Last(d), Record({2018, 12, 22}, "e2"), "Db Last 4");

        AssertEqual(db.Last(d1), Record({2019, 1, 2}, "e1"), "Db Last 5");
        db.Add(d2, "e4");
        AssertEqual(db.Last(d2), Record({2018, 12, 22}, "e4"), "Db Last 6");
    }
